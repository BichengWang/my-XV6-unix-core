#include "types.h" #include "defs.h"
#include "param.h"
#include "memlayout.h"
#include "mmu.h"
#include "x86.h"
...
int mycopybuffer(pde_t *pgdir, char * addr, char * buffer, uint offset, uint buffersize){
	int guardFlag = 0;
	uint i, pa, n;
	pte_t * pte;
...
int clone(void (*fcn)(void *), void * arg, void * stack) {
  ...
  // np->isthread = 1;
  // modified the stack
  // np->context->esp = (int)stack;
	np->tf->esp = (uint)stack + 4092; // move esp to the top of the new stack
  *((uint *)(np->tf->esp)) = (uint)arg; // push the argument
  *((uint *)(np->tf->esp - 4)) = 0xFFFFFFFF; // push the return address
	np->tf->esp -= 4;

  // Copy file descripor table
  for(i = 0; i < NOFILE; i++)
    if(proc->ofile[i])
